...
public ResponseEntity<Page<IncidentFields>> findPaginatedIncidentsByFilters(
...
if (!companyIdFromToken.equals(company)) {
    ...
    // Regla especial para created_on / updated_on (según tu lógica original)
    String fieldLower = field.toLowerCase();
    if ("created_on".equals(fieldLower) || "updated_on".equals(fieldLower)) {
        if ("DESC".equalsIgnoreCase(direction)) {
            sort = Sort.by(field).ascending();
        } else {
            sort = Sort.by(field).descending();
    ...
    Pageable pageRequest = (size != null && size > 0)
            ? PageRequest.of(page, size, sort)
            : SortedUnpaged.getInstance(sort);

    log.info(pageRequest.toString());
    log.info("Field Criteria: " + field);

    // ================== SERVICE CALL ==================
    Page<IncidentFields> pageResult = incidentService.findPaginatedIncidentsByFilters(
            pageRequest,
            filter.toUpperCase(),
            assignedTo,
            company,                 // <- ya validado/ajustado con el token
            state,
            openUnassigned,
            solved,
            incidentParent,
    ...
    for (IncidentFields elemento : contenido) {
        log.debug("sysInfo: " + elemento.getNumber() + ", " + elemento.getShort_description() + ", " +
                elemento.getAssigned_to() + ", " + elemento.getCategory() + ", " + elemento.getId() + ", " +
                elemento.getState() + "," + elemento.getCreated_on() + ", " + elemento.getUpdated_on());
    }
